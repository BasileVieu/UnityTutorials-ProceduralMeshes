...
namespace ProceduralMeshes.Generators
{
    public struct Icosphere : IMeshGenerator
    {
        ...
        public void Execute<S>(int _i, S _streams) where S : struct, IMeshStreams
        {
            ...
            float3 columnLowStart = strip.lowRightCorner + columnLowDir * ((float)u / Resolution - 1.0f);
            ...
            float3 columnTopStart = strip.highRightCorner + columnTopDir * ((float)u / Resolution - 1.0f);
            ...
            for (int v = 1; v < ResolutionV; v++, vi++, ti += 2)
            {
                ...
                {
                    vertex.position = lerp(columnHighStart, columnHighEnd, (float)v / Resolution - 1.0f);
                }
                else
                {
                    vertex.position = lerp(columnTopStart, columnTopEnd, (float)v / Resolution - 1.0f);
                }
        ...
        static float3 GetCorner(int _id, int _ySign) => float3(0.4f * sqrt(5.0f) * sin(0.2f * PI * _id),
                                                   _ySign * 0.2f * sqrt(5.0f),
                                                   -0.4f * sqrt(5.0f) * cos(0.2f * PI * _id));

        static Strip GetStrip(int _id) => _id switch
        {
            0 => CreateStrip(0),
            1 => CreateStrip(1),
            2 => CreateStrip(2),
        ...
        static Strip CreateStrip(int _id) => new Strip
        {
            id = _id,
