...
namespace ProceduralMeshes.Generators
{
    public struct FlatHexagonGrid : IMeshGenerator
    {
        ...
        public Bounds Bounds => new Bounds(Vector3.zero, new Vector3(0.75f + 0.25f / Resolution, 0.0f, (Resolution > 1 ? 0.75f / Resolution : 0.5f) * sqrt(3.0f)));

        public void Execute<S>(int _x, S _streams) where S : struct, IMeshStreams
        {
            int vi = 7 * Resolution * _x;
            int ti = 6 * Resolution * _x;
            ...
            {
                float2 center = (float2(0.75f * _x, 2.0f * h * z) + centerOffset) / Resolution;
                ...
                vertex.texCoord0 = float2(1.0f, 0.5f);
